    ...
    <div className="w-full min-h-[calc(100vh-6rem)] bg-[var(--background)] p-4 lg:p-8 rounded-xl flex flex-col gap-8">
      ...
      <div className="space-y-4">
        {orders.map((order) => (
          <div
            key={order.id}
            className="group bg-white border border-[var(--border)] rounded-xl p-6 hover:shadow-md transition-shadow"
          >
            {/* Order Header */}
              ...
              <div className="relative flex justify-between text-xs font-medium text-[var(--text-muted)]">
                {["Placed", "Packed", "Shipped", "Delivered"].map(
                  (step, idx) => (
                    <div
                      key={step}
                      className="flex flex-col items-center gap-2 bg-white px-1 z-10"
                    >
                      <div
                        className={`w-3 h-3 rounded-full border-2 ${
                          idx + 1 <= order.statusStep
