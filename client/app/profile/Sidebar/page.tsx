...
export default function ProfileSidebar() {
  ...
  const activeItem = menuItems.find(item => item.href === pathname) || { label: "Menu" };
        ...
        {isMobileMenuOpen && (
          <nav className="mt-4 pt-4 border-t border-white/10 flex flex-col gap-2 animate-in slide-in-from-top-2 fade-in duration-200">
            {menuItems.map((item) => {
             ...
             <button className="flex items-center gap-3 px-4 py-3 rounded-xl text-red-400 hover:bg-white/5 transition-all w-full text-left mt-2">
                <LogOut size={20} />
                <span>Sign Out</span>
             </button>
          </nav>
          ...
          {menuItems.map((item) => {
              ...
              return (
                  ...
                  >
                      ...
                      <item.icon size={20} className={`z-10 transition-transform duration-300 ${isActive ? "scale-110" : "group-hover:scale-110"}`} />
